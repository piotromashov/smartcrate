...
- [x] 9.2 Run the full loop: Play → playback → like/dislike/skip → scores update → queue refills → liked track downloads locally via `yt-dlp` — verified live: liking the current track updated entity scores and `yt-dlp`+`ffmpeg` produced an 8 MB MP3 320 at `downloads/<trackId>.mp3` (status `done`)
